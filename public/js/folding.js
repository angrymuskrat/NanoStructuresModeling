...
function buildHex(points, solve, ind0, ind1, ind2) {
    ...
    for (let i = 2; i < 6; i++) {
        hex.push(makePoint(hex[i - 2], hex[i - 1], indC));
    }
...
function buildup(cubs, size, points, r0, ri, alpha, a) {
    points = points.map(array => array.map(point => point.toPolarPoint()));
    let riPi = ri / (2 * Math.PI);
    let g = Math.tan(alpha);
    let solve = (p0, p1, p2) => {
    ...
    let makeHex = (i0, j0, i1, j1, i2, j2) => buildHex(points, solve, makeElem(i0, j0), makeElem(i1, j1), makeElem(i2, j2));
    let last = (points[0].length - 1) - (points[0].length - 1) % 3;
